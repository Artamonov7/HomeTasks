...
void delete_list(struct token_list *);
...
struct token_list *add_to_start(struct token_list *first, char *str)
{
...
struct token_list *reverse_list(struct token_list *first)
{
    struct token_list *reversed_list = NULL, *saved = first;
    while (first){
        char *str = strdup(first->token);
        reversed_list = add_to_start(reversed_list, strrev(str));
        first = first->next;
    }
    delete_list(saved);
